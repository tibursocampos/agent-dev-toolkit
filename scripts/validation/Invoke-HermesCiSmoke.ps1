...
<#
.SYNOPSIS
  In-repo Hermes CI smoke (sync + validate against fixture seed).

.DESCRIPTION
  Copies the versioned Hermes fixture into an ephemeral work InstallRoot, then
  runs sync-agent + validate-agent for -Agent hermes. Does not require a Hermes
  profile, CLI, or USERPROFILE writes. Never passes -AllowUserHome. The
...
$suiteTitle = 'agent-dev-toolkit Hermes CI smoke'
$agentId = 'hermes'
$seedFixtureRel = 'scripts\validation\fixtures\hermes'
$workFixtureRel = 'scripts\validation\fixtures\hermes-ci-smoke'
$suitePassMarker = 'Invoke-HermesCiSmoke: PASS'
