...
<#
.SYNOPSIS
  In-repo Grok CI smoke (sync + validate against fixture seed).

.DESCRIPTION
  Copies the versioned Grok fixture into an ephemeral work InstallRoot, then
  runs sync-agent + validate-agent for -Agent grok. Does not require a Grok
  profile, trust UI, or USERPROFILE writes. Never passes -AllowUserHome. The
...
$suiteTitle = 'agent-dev-toolkit Grok CI smoke'
$agentId = 'grok'
$seedFixtureRel = 'scripts\validation\fixtures\grok'
$workFixtureRel = 'scripts\validation\fixtures\grok-ci-smoke'
$suitePassMarker = 'Invoke-GrokCiSmoke: PASS'
