...
3. **Manifest** - read `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/codex/sdd/<repo-id>/manifest.json` when it exists and `workspace_root` (normalized separators, case-insensitive on Windows) matches the open workspace -> derive classic feature root from `STORAGE.md` (repository `features/` or global `.../features/`).
4. **Glob** (parallel) - **only** under `features/` (never root/flat `PRD/` or `PLAN/`):
   ...
   | Global | `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/codex/sdd/<repo-id>/features/**/PRD/*.md`, `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/codex/sdd/<repo-id>/features/**/PLAN/PLAN_*.md` |
