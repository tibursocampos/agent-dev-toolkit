...
Use the **same** SDD PLAN path `sdd-develop` passed in (`features/**/PLAN/PLAN_*.md` or global `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/install-root/sdd/<repo-id>/features/**/PLAN/` per `STORAGE.md`). Do not use root/flat `PLAN/` or `docs/documentation-plan/plan.md`.
