...
Full detail: `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/grok/skills/_shared/sdd-artifacts/PIPELINE.md` (load when running `sdd-spec`, `sdd-plan`, or `sdd-develop`).
...
- PRD: `features/NNN-slug/USnn/PRD/NNN_*.md` (default story `US01`) or global under `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/grok/sdd/<repo-id>/features/...`.
...
- `sdd-develop`: **one PLAN step per develop session** (unchanged contract). Develop gates (`step_confirmed`, `tests_run`) live in PLAN-scoped files under `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/grok/sdd/sessions/{repo-hash}/` - see `SESSION.md` (supports parallel O3 without sharing one flat session JSON).
