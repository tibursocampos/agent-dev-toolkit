...
| Cursor mode (Agent for shell) | `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/grok/skills/_shared/sdd-artifacts/PIPELINE.md` section Cursor mode |
...
| Add tests for gaps | `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/grok/skills/_shared/dotnet-guidelines/csharp-patterns.md` |
| Commit | `/commit` |
...
Confirm **target .NET repository** (`.sln` or `*Tests.csproj`). If the workspace is `agent-dev-toolkit` only (no test projects), stop and ask which consumer repo to open.
