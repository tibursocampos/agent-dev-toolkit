...
| **Location (shared)** | Public/shared type (e.g. `Constants` **partial** under Models/Domain) only when the value is **reused or clearly intended for reuse** across classes (CT6, RN06) |
| **Shared file size** | Watch growth of shared constants types; prefer **`partial class`** by responsibility and section grouping via **`#region`** or comment headers - avoid a single monolithic constants dump |
| **No duplication** | Do not copy the same literal into multiple classes or constant files - one canonical definition |
| **Review** | **Blocking** - request named extraction before merge (CT5); also flag unnecessary shared-file churn for one-off values |
...
// Correct - local one-off: private const on the owning class (not a shared Constants file)
...
// Correct - shared only when reused across types (prefer partial by responsibility)
...
Within each class, organize **methods** (PRD CA5, RF05, RN04) in this **exact sequence** - agents, implementation, and code review must obey it:

1. All **public** methods, ordered **alphabetically by method name** (case-insensitive).
2. Then all **private** methods, ordered **alphabetically by method name** (case-insensitive).

| Rule | Detail |
|------|--------|
| **Visibility blocks** | All **public** methods before all **private** methods - never interleave |
...
| **Review / implementation** | **Blocking** - request reorder before merge; do not ship new methods at the bottom of the wrong block |

**Sort key:** Use the method name only (ignore parameters). Applies to instance and static methods in each visibility block.

**Discover first:** If the class or feature folder already uses a documented ordering (e.g. constructors grouped at the top of the public block), match that **existing project pattern** after Glob/Read - do not fight a consolidated local convention unless the PR standardizes it. Otherwise apply the public-then-private alphabetical sequence above.
...
- Do **not** use `ConfigureAwait` in application code (ASP.NET Core APIs, workers, handlers). Prefer plain `await` end-to-end and propagate `CancellationToken`.
...
// Wrong - deadlock risk / sync-over-async
public Order GetOrder(int id)
{
    return _orderRepository.GetByIdAsync(id).Result;
}

// Wrong - do not sprinkle ConfigureAwait in app code
return await _orderRepository.GetByIdAsync(id, cancellationToken).ConfigureAwait(false);
```

### Modern C# (primary constructors, collection expressions, `required`)

- **New types / new files** with no established local style: prefer primary constructors, collection expressions (`[1, 2, 3]`), and `required` members when they fit the type.
- **Existing code / repo already using classic constructors or array syntax:** match the project - do not rewrite only to modernize.

```csharp
// Preferred for new types
...
| **Shouldly** | Assertions via `ShouldBe`, `ShouldNotBeNull`, `ShouldBeTrue`, etc. |
| **Moq** | Mocks via `new Mock<T>()` / `Mock.Of<T>()` |
| **WireMock.Net** | HTTP stubbing in integration/infrastructure tests (when needed) |

Prefer **xUnit + Moq + Shouldly** for all new tests. Legacy NUnit/NSubstitute/FluentAssertions tests may remain; do not rewrite them unless asked - add new scenarios in new files following this stack.
...
// Avoid FluentAssertions `.Should().*` in new tests (use Shouldly)
```
...
| 5 | Classic `Assert.*` or FluentAssertions in new tests | Use Shouldly (`ShouldBe`, `ShouldNotBeNull`, …) |
| 6 | Test name not `Should_*_When_*` | Rename |
| 7 | `.Result` / `.Wait()` on tasks | Use `await` |

**Accepted patterns:** `IClassFixture<T>`, `CreateScope()`, `*Fake.*`, `Mock<T>`, Shouldly, `[Theory]`, `Should_*_When_*`.
