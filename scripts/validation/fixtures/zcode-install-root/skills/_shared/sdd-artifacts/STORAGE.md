...
Install path after sync: `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/skills/_shared/sdd-artifacts/STORAGE.md`
...
| **global** | `<path>/features/NNN-slug/` where `<path>` = `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd/<repo-id>/` (or manifest `classic.path`) | `<path>/memory-bank/` |

Use `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd/...` on macOS/Linux when expanding paths in tools.
...
4. If a baked absolute `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd` in the loaded SKILL points under a **different** agent home than `effective_SDD_ROOT`, **ignore the baked path** and use `effective_SDD_ROOT`. Optionally warn once in chat (pt-BR): `Skills de outro agente detectadas; usando SDD do host atual.`
...
Core source keeps placeholders `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd`; publish may still bake absolutes. Runtime **must** apply this override when the baked path's agent home ≠ the current host agent home.
...
(Docs and core source may still write `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd/manifest.json`; at runtime expand to `effective_SDD_ROOT`.)
...
```json
{
  ...
  "repositories": {
    "D:/Source/Repos/MyApp": {
      "classic": {
        "storage_mode": "global",
        "path": "E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd/MyApp"
      }
    }
  }
}
...
2) Global - E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/sdd/<RepositoryName>/ (features/ + memory-bank/ fora do git do projeto; sem alterar .gitignore)
```
