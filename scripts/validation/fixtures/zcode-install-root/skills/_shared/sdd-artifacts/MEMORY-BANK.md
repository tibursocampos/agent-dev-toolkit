...
Install path after sync: `E:/Source/Repos/agent-dev-toolkit/scripts/validation/fixtures/zcode-install-root/skills/_shared/sdd-artifacts/MEMORY-BANK.md`
...
**Classic SDD (Classic SDD):** memory-bank is **optional** - not required for `sdd-spec` / `sdd-plan` / `sdd-develop` (**CA7**). Phase 2 BLOCKING / ARCH point-promote apply to Orchestrated Delivery (`orchestrate-*`) and to promote; Classic SDD does not require the bank.
