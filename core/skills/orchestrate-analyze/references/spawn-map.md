...
Spawn a Task subagent **only** when `subagents=native` **and** `ROSTER.md` canonical `needs_*` / brownfield rules say so. Load prompt from `skills/_shared/agents/prompts/`. The parent prompt requires the return shape in `refine-story/references/finding-format.md` (zero or more finding blocks; a summary is not the only product). Who is called does not change. When multiple specialists apply, spawn **in parallel** within the SPAWN ≤4 cap; if more flags apply, batch in waves of ≤4 or ask in the user chat language to run série.
