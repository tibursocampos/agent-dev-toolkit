...
| Reference index (routing only) | `{{TOOLKIT_ROOT}}/skills/vue-developer/reference.md` |
| Process step detail (lazy) | `{{TOOLKIT_ROOT}}/skills/vue-developer/references/<section>.md` |

Do **not** preload unrelated guideline trees. Do not redesign `vue-guidelines/` in this skill — load existing pack rows only.

**Never by default:** do not preload all `references/*.md`. do not preload other stack guideline packs. Load only Vue rows needed for the current task. Do not dump full stack guideline packs or memory-bank. Load **one** section per Process step (`SKILL-REFERENCE-RETRIEVAL.md`).
