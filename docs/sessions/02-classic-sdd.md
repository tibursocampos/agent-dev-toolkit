...
A PLAN exists only after a canonical PRD whose status is ready for planning. The PLAN number matches the PRD number and lives in the **same** story folder. Each step is sized for one later `sdd-develop` session (about 20–45 minutes of work). Steps do not contain code blocks.
...
3. Map every `REQ-NNN` to a step. Each step acceptance cites a REQ or acceptance id and an observable outcome. Step titles are not only a file, class, or script name.
4. Confirm path and step count. **sim** before write.
5. Write steps as pending, progress `0/N`, refresh `## Related` so PRD and PLAN cite each other.
6. Run `validate-plan` (and `validate-prd` on the source). Failure blocks `/sdd-develop`.
...
2. Confirm the step exists and its dependencies are completed. Summarize files, tests, and REQ targets. Ask to proceed.
...
8. Update the PLAN in place (status, progress, next step). Check acceptance only when the cited REQ is actually met. Do not record duration estimates. Preserve `## Related`.
9. Optional `/commit` after the living-artifact questions (bank refresh-light, project docs) when those trees exist. Silence does not count as skip.

If the parent is `orchestrate-develop`, the child does not own `CONTINUITY.md`. The parent writes that file after the receipt.

Handoff when the PLAN is finished: ask `code-review` versus `commit`. Review modes: [05](05-review-and-quality.md).
...
|--------|----------------------------------|
| `orchestrate-deliver` series | Parent runs spec then plan, writes after **sim**, repeats per story |
