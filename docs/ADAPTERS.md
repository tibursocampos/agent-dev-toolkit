...
| `scripts/sync-agent.ps1 -Agent <id>` | Loads registry entry + module; calls `Publish-*`. Unknown or incomplete adapters exit ≠ 0 with an explicit **not implemented** message (TE04). Defaults `InstallRoot` to in-repo fixture; refuse USERPROFILE without `-AllowUserHome`. |
...
| `sdd` | Can publish/resolve SDD contracts (`Get-SddRoot`) |
...
Honesty matrix (Tier 1 **registry**): all eight agents declare `subagents: native` (host product docs), including **Antigravity**. **Antigravity** *effective* capability is fail-closed via `Get-Capabilities` probe (`ADT_ANTIGRAVITY_SUBAGENTS` / `agy` / product version) — pré-2.0 or unverifiable → `none`. `validate-core` checks registry, each module’s `Get-Capabilities` (Antigravity with CI override), orchestrate SPAWN/fallback text, and Antigravity probe cases. CI adapter smokes stay filesystem sync/validate — no duplicate spawn matrix there.

The **cursor** registry entry sets `skills` / `rules` / `hooks` / `router` / `sdd` to `true` and `plugin` to `false`; `Get-Capabilities` on `adapters/cursor/CursorAdapter.ps1` mirrors those flags with `Implemented = true`. The **claude** registry entry sets `skills` / `rules` / `hooks` / `router` to `true` and `sdd` / `plugin` to `false` (honest: Claude MVP does not claim SDD publish parity; hooks smoke validates files only — trust UI out of scope). The **antigravity** module `Get-Capabilities` reports `skills` / `rules` / `router` / `plugin` = `true` and `hooks` / `sdd` = `false` (honest: no native shell-hook parity with Cursor; registry flags may lag until a later wire-up step). The **copilot** registry entry sets `skills` / `rules` / `hooks` to `true` and `router` / `sdd` / `plugin` to `false` (`Publish-Router` is a documented no-op; router guidance folds into `copilot-instructions.md` via `Publish-Policy`). The **opencode** module `Get-Capabilities` reports `skills` / `hooks` / `router` / `plugin` = `true` and `rules` / `sdd` = `false`, with `HooksSemantics=plugin-only` (JavaScript plugins under `plugins/` — not shell/PS1 hooks). The **grok** registry entry sets `skills` / `rules` / `hooks` / `router` to `true` and `sdd` / `plugin` = `false` (native `.grok` publish; hooks trust UI out of smoke/CI). The **codex** registry entry sets `skills` / `hooks` / `router` / `plugin` to `true` and `rules` / `sdd` to `false` (Codex plugin packaging; hooks trust via `/hooks` is manual — out of smoke/CI).
...
`Uninstall-Toolkit` is **not implemented** for Cursor (fail-closed stub — no filesystem writes). Keyed uninstall applies to other Tier 1 agents that implement it; see CI keyed uninstall asserts.
...
| `Uninstall-Toolkit` | Keyed removal of toolkit artifacts only (core skill folders, `dev_persona`, managed plugin dir, managed `skills.json` entry, managed markdown blocks). Preserves alien skills / hooks / legacy bridge. |
...
| Fixture override | `-InstallRoot <path>` (CI default: in-repo fixture; USERPROFILE paths require `-AllowUserHome`) |
| Capabilities | `skills` / `rules` / `hooks` = true; `router` / `sdd` / `plugin` = false |
...
| Uninstall | Keyed removal of toolkit skills / instructions / hooks only (RN07 — no wholesale wipe of `~/.copilot` or `.github`) |
...
Keyed removal of toolkit skills / rules / hooks / `CLAUDE.md`, plus reverse-merge of managed hooks events and toolkit `permissions.allow` entries. Alien files and unrelated settings keys stay. WhatIf supported. Module notes: `adapters/claude/README.md`.
...
| `Uninstall-Toolkit` | Keyed removal of toolkit artifacts only (no wipe of `plugin/` / `.agents` / alien files) |
...
| Uninstall | Keyed removal of toolkit skills, `AGENTS.md`, and the marker plugin only (RN07 — no wholesale wipe) |
...
`Uninstall-Toolkit` is **not implemented** for ZCode (fail-closed stub — no filesystem writes).
...
| `Uninstall-Toolkit` | Keyed removal of toolkit artifacts only (no wipe of `.grok` / `config.toml`) |
...
| `Get-SddRoot` | Resolve published SDD root | Not-implemented result; source of truth remains `core/sdd/` |
| `Invoke-SmokeValidate` | Smoke against fixture `InstallRoot` | Not-implemented result; must not require live user-profile sync |
| `Uninstall-Toolkit` | Remove published toolkit files | Not-implemented result; **no** filesystem writes |
...
- Concrete adapters publish from `core/sdd/` and return that published root from `Get-SddRoot`.

### Cursor (`cursor`) — SDD root
