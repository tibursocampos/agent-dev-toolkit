...
Portable contracts such as `PIPELINE.md`, `STORAGE.md`, `SESSION.md`, `MEMORY-BANK.md`. Source of truth for pipeline text. Runtime state (e.g. Cursor `sdd/sessions/`, `manifest.json`) is prepared by adapters with `sdd=true` via `Get-SddRoot -Prepare`.
