...
2. **Operators** — people who already use Cursor, Claude Code, Copilot, Codex, or another Tier-1 agent; they need the right sync/validate commands, agent install roots, and which Forma/skill to invoke.
...
Success: a visitor understands the value in one scroll, picks their agent, copies a sync command, and reaches a working skill invoke (e.g. `/sdd-spec`) without reading the whole repo.

## Brand Personality

**precise · multi-agent · operator-first**

Voice: confident, concrete, command-oriented. Prefer “run this / get that” over abstract AI marketing. Emotional goals: clarity and trust (safe defaults, fixture-first CI), curiosity (eight agents, one core), competence (not playful gimmicks).
