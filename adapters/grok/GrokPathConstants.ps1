...
$script:GrokAdapterConstant = @{
    ...
    OfficialCustomAgentsDescription  = 'Custom agent markdown publishes under agents/*.md relative to InstallRoot (live ~/.grok/agents/).'
    ...
    HooksPreToolUseMatcher           = 'Write|Edit|Bash'
    HooksCommandType                 = 'command'
    HooksSessionStartCommandTemplate = 'pwsh -NoProfile -File "{0}"'
    HooksSessionStartStatusMessage   = 'Grok toolkit SessionStart hook (filesystem publish; trust is manual)'
    HooksDescription                 = 'agent-dev-toolkit Grok SessionStart + PreToolUse path/secrets guards (docs.x.ai/build/features/hooks). Trust via /hooks-trust or --trust is manual; smoke never writes trusted_folders.toml.'
...
$script:GrokAdapterMessage = @{
    ...
    HooksPublishedOk          = 'Grok Publish-Hooks: published native JSON + PreToolUse guard under {0} (filesystem only; trust /hooks-trust or --trust is manual)'
    ...
    HooksAssetsMissing        = 'Grok Publish-Hooks: hook script asset is missing: {0}'
    CoreAgentsMissing         = 'Grok Publish-Agents: core agents source is missing: {0}'
    AgentsPublishedOk         = 'Grok Publish-Agents: published {0} agent file(s) from core/agents to {1}'
    AgentsWhatIfOk            = 'Grok Publish-Agents: WhatIf - would publish {0} agent file(s) to {1}'
    ...
    SmokeTe03AgentsMissing    = 'Grok Invoke-SmokeValidate TE03: agents capability is true but roster markdown missing under {0}.'
