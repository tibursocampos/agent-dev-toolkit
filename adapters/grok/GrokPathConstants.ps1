...
$script:GrokAdapterMessage = @{
    ...
    CapabilitiesReady         = 'Grok adapter capabilities reported (skills/rules/hooks/router via native .grok). Publish-Skills/Policy/Router/Hooks and Invoke-SmokeValidate ready; Uninstall-Toolkit removes keyed toolkit artifacts only (no wipe of .grok / config.toml). Hooks trust UI /hooks-trust out of smoke scope. SDD runtime prepared on sync.'
