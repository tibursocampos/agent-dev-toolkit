...
| Also | Customize â†’ Skills (picker / auto) |
...
Public `Publish-Skills` (etc.) in `CursorAdapter.ps1` forward to `Invoke-Cursor*` implementations â€” same pattern as Claude/Grok.
...
| Flag | Value | Notes |
|------|-------|-------|
| `skills` | true | `core/skills` â†’ `skills/<id>/SKILL.md` |
| `rules` | true | `core/policy` â†’ `rules/*.mdc` |
| `hooks` | true | Scripts under `hooks/` + root `hooks.json` merge |
| `router` | true | `core/router/AGENTS.md` â†’ `AGENTS.md` |
| `plugin` | false | â€” |
| `agents` | true | `core/agents` â†’ `agents/*.md` (Task targets) |
...
| Published files | `Publish-Agents` copies `core/agents/` â†’ `InstallRoot/agents/` (live `~/.cursor/agents/`). Roster: `repo-analyst`, `architect`, `database`, `security`, `shell-runner`. Parent stays the main agent. |
| Model inherit (REQ-008) | `model: inherit` required on published agents; Publish asserts after copy. |
| Depth / threads | SPAWN caps ≤2 / ≤4; not pinned as alternate Task `model` slugs. |
...
When this toolkit is installed **per agent** via sync (separate Cursor and Claude/Codex installs), **do not** let Cursor load the other agentsâ€™ skills or configs.

1. **Recommendation:** In Cursor Settings â†’ Rules, Skills, Subagents, **disable** **Include third-party Plugins, Skills, and other configs**.
...
| Field | Value |
|-------|-------|
| Events | `postToolUse`, `subagentStop` → `hooks/emit-trace.ps1` |
| Contract | Fail-open; allowlist via `TraceEmitCommon.ps1`; SoT `features/NNN-slug/TRACE.jsonl` |
